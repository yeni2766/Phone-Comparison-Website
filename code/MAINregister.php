...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["Password"]))){
		$success = false;
		$formmessages= ["Please enter a password."];  
	elseif(strlen(trim($_POST["Password"])) < 7){
		$success = false;
      $formmessages  = ["Your password must have 7 characters at least to create an account."];
    ...
    if(empty(trim($_POST["confirmpassword"]))){
		$success = false;
      $formmessages = ["Please confirm password."];  
    } else{
        $pwdrepeat = trim($_POST["confirmpassword"]);
        if ($Password != $pwdrepeat){
			$success = false;
              $formmessages ["Password did not match. please try again"];
        }
    }
...
if(empty(trim($_POST["email"]))){
		$success = false;
        $formmessages = ["Please enter a email."];
    } else{
    ...
    // joins the variables together to be used as parameters
            ...
            if(mysqli_stmt_execute($result)){
                ...
                if(mysqli_stmt_num_rows($result) == 1){
					$success = false;
                    $formmessages = ["This email is already taken. please enter another email"];
                } else{
					$success = true;
				...
				$success = false;
              $formmessages ["Please try again later, something has gone wrong"];
            }
...
if(empty(trim($_POST["firstname"]))){
		$success = false;
        $formmessages = ["Please enter a First Name."];
    } else{
           ...
           // joins the variables together to be used as parameters
            ...
            if(mysqli_stmt_execute($result)){
               ...
               $success = true;
				...
				$success = false;
        $formmessages =["Please try again later, there has been a connection problem"];
            }
...
if(empty(trim($_POST["LastName"]))){
	$success = false;
        $formmessages = ["Please enter a Last name."];
    } else{
            ...
            if(mysqli_stmt_execute($result)){
               ...
               $success = true;
                 $lastname = trim($_POST['LastName']);
					...
					$success = false;
              $formmessages =["Please try again later. Something is wrong at the moment"];
            }
